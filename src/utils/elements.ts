import { universe, celestial } from "map"


const mapWrapper = (map: string) => `<p>
...
const coordinate = (celestial: celestial) => {
    const wrap = (icon: string) => `<span class="">${icon}</span>`
    const getIcon = (celestial: celestial) => {
        switch (celestial) {
            case "POLYANET":
                return wrap("🪐")
            case "SPACE":
